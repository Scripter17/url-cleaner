...
window.config = {
	instance_origin: "http://localhost:9149", // The origin (protocol://host:port) of your URL Cleaner Site instance. When changing, please also update the "// @connect" line above.
...
async function clean_elements(elements, job_config) {
	...
	if (job_config.tasks.length == 0) {return;}
...
async function get_host_parts(host) {
	...
	let doneawaiter = new Promise(resolve => {done = resolve;});
	await GM.xmlHttpRequest({
		url: `${window.config.instance_origin}/get-host-parts`,
		method: "POST",
