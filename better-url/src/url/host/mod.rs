...
impl BetterUrl {
    ...
    pub fn has_host(&self) -> bool {
        self.details.host_start.is_some()
    }

    /// The [`Range::start`] of the host.
    fn host_start(&self) -> Option<usize> {
        Some(self.details.host_start?.get() as usize)
    }

    /// The [`Range::end`] of the host.
    pub(crate) fn host_after(&self) -> Option<usize> {
        if self.details.host_start.is_some() {
            Some(self.details.port_mark.map_or(self.details.path_start, NonZero::get) as usize)
        } else {
            None
        }
    }
    ...
    pub fn host_str(&self) -> Option<&str> {
        Some(unsafe {self.as_str().get_unchecked(self.host_range()?)})
    }

    /// The [`Host`].
    pub fn host(&self) -> Option<Host<'_>> {
        unsafe {Some(Host::new_unchecked(self.host_str()?, self.host_details()?))}
    }
        ...
        where InvalidHost: From<<T as TryInto<FileHost<'a>>>::Error> + From<<T as TryInto<SpecialNotFileHost<'a>>>::Error> + From<<T as TryInto<NonSpecialHost<'a>>>::Error>
    {
        ...
        match self.host_range() {
            None => {
                if self.details.path_start == self.details.scheme_mark + 3 {
                    let diff = new.len();
                    ...
                    if let Some(x) = self.details.port_mark     {self.details.port_mark     = NonZero::new(x.get() + diff as u32);}
            ...
            Some(range) => {
                if new.is_empty() && (self.details.username_after.is_some() || range.end != self.details.path_start as usize) {
                    Err(CantBeNone)?;
                }
