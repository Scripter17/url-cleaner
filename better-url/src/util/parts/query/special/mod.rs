...
/// Encode a [`SpecialQuery`].
pub fn encode_special_query_bytes<'a, T: Into<Cow<'a, [u8]>>>(value: T) -> (bool, Cow<'a, str>) {
    percent_encode_bytes(value, SPECIAL_QUERY)
}

/// Encode a [`SpecialQuerySegment`].
pub fn encode_special_query_segment_bytes<'a, T: Into<Cow<'a, [u8]>>>(value: T) -> (bool, Cow<'a, str>, Option<NonZero<usize>>) {
    let (changed, value) = percent_encode_bytes(value, SPECIAL_QUERY_SEGMENT);
