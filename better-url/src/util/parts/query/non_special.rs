...
pub fn encode_non_special_query<'a, T: Into<Cow<'a, str>>>(value: T) -> (bool, Cow<'a, str>) {
    percent_encode(cow_str_to_bytes(value.into()), false, false, false, NON_SPECIAL_QUERY)
}
...
pub fn encode_non_special_query_segment<'a, T: Into<Cow<'a, str>>>(value: T) -> (bool, Cow<'a, str>, Option<NonZero<usize>>) {
    let (changed, value) = percent_encode(cow_str_to_bytes(value.into()), false, false, false, NON_SPECIAL_QUERY_SEGMENT);
...
pub fn fragment_to_non_special_query<'a, T: Into<Cow<'a, str>>>(value: T) -> (bool, Cow<'a, str>) {
    percent_encode(cow_str_to_bytes(value.into()), false, false, false, FRAGMENT_TO_NON_SPECIAL_QUERY)
}
