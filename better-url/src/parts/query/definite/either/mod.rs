...
/// Either a [`SpecialQuery`] or a [`NonSpecialQuery`].
///
/// Constructors prefer [`SpecialQuery`].
#[derive(Debug, Clone)]
pub enum Query<'a> {
    /// [`SpecialQuery`].
...
impl<'a> From<Cow<'a, str>> for Query<'a> {fn from(value: Cow<'a, str>) -> Self {Self::Special(value.into())}}
...
impl<'a> From<Fragment<'a>> for Query<'a> {fn from(value: Fragment<'a>) -> Self {Self::Special(value.into())}}
