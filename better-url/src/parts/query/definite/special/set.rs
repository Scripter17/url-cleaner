...
impl SpecialQuery<'_> {
    /// Finds the `index`th segment whose [`SpecialQueryName::decode`] is `name` and does stuff.
    ///
    ...
    /// ```
    pub fn set<'b, N: AsRef<[u8]>, T: Into<MaybeSpecialQueryValue<'b>>>(&mut self, name: N, index: isize, value: Option<T>) -> Result<bool, SetQueryError> {
        Ok(match value.map(|x| x.into().into_inner()) {
            Some(Some(value)) => {
                ...
                match temp {
                    ...
                    Err(0) => {
                        let name = SpecialQueryName::new(name.as_ref()).into_inner();
