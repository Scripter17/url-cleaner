...
impl<'a> NonSpecialQuery<'a> {
    ...
    /// A [`DoubleEndedIterator`] of the [`NonSpecialQuerySegment`]s whose [`NonSpecialQueryName::decode`] is `name`.
    ...
    /// The `index`th [`NonSpecialQuerySegment`] whose [`NonSpecialQueryName::decode`] is `name`.
