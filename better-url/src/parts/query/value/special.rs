...
///
/// However, the only invariant is that this is a substring of a [`SpecialQuery`] that contains no `&` literals.
#[derive(Debug, Clone)]
pub struct SpecialQueryValue<'a>(Cow<'a, str>);

impl<'a> SpecialQueryValue<'a> {
    ...
    pub fn borrowed(&self) -> SpecialQueryValue<'_> {
        SpecialQueryValue(self.as_str().into())
    }

    /// Turn into an owned [`Self`].
    pub fn into_owned(self) -> SpecialQueryValue<'static> {
        SpecialQueryValue(self.0.into_owned().into())
    }
...
impl<'a> From<QueryLikeValue<'a>> for SpecialQueryValue<'a> {
...
impl<'a> From<QueryValue<'a>> for SpecialQueryValue<'a> {
    fn from(value: QueryValue<'a>) -> Self {
        match value {
            QueryValue::Special   (x) => x,
            QueryValue::NonSpecial(x) => x.into(),
        }
    }
}

impl<'a> From<FragmentQueryValue  <'a>> for SpecialQueryValue<'a> {fn from(value: FragmentQueryValue  <'a>) -> Self {unsafe {Self::new_unchecked(fragment_to_non_special_query     (value.into_inner()).1)}}}
impl<'a> From<NonSpecialQueryValue<'a>> for SpecialQueryValue<'a> {fn from(value: NonSpecialQueryValue<'a>) -> Self {unsafe {Self::new_unchecked(non_special_query_to_special_query(value.into_inner()).1)}}}
