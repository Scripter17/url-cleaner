...
impl MaybeQuery<'_> {
    /// [`Query::set`].
    ///
    /// If [`Self::0`] is [`None`], `index` is 0 or 1, and `value` is [`Some`], creates a new [`Query`] with [`QuerySegment::from_pair`].
    ///
    ...
    pub fn set(&mut self, name: &str, index: isize, value: Option<Option<&str>>) -> Result<bool, SetQueryError> {
        match &mut self.0 {
            ...
            None => match (index, value) {
                (0 | -1, Some(value)) => {self.0 = Some(QuerySegment::from_pair(name, value).into_owned().into()); Ok(true)},
