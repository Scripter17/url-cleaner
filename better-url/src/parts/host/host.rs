...
impl<'a> Host<'a> {
    ...
    /// The [`DomainHost::as_str`].
    pub fn domain_str(&self) -> Option<&str> {
        match self.is_domain() {
            true  => Some(self.as_str()),
            false => None
        }
    }
    ...
    pub fn domain_prefix(&self) -> Option<&str> {
        Some(&self.as_str()[self.as_domain()?.details().prefix_range()?])
    }

    /// The [`DomainHost::middle`].
    pub fn domain_middle(&self) -> Option<&str> {
        Some(&self.as_str()[self.as_domain()?.details().middle_range()?])
    }

    /// The [`DomainHost::suffix`].
    pub fn domain_suffix(&self) -> Option<&str> {
        Some(&self.as_str()[self.as_domain()?.details().suffix_range()])
    }

    /// The [`DomainHost::labels`].
    pub fn domain_labels(&self) -> Option<&str> {
        Some(&self.as_str()[self.as_domain()?.details().labels_range()])
    }

    /// The [`DomainHost::origin`].
    pub fn domain_origin(&self) -> Option<&str> {
        Some(&self.as_str()[self.as_domain()?.details().origin_range()?])
    }

    /// The [`DomainHost::normal`].
    pub fn domain_normal(&self) -> Option<&str> {
        Some(&self.as_str()[self.as_domain()?.details().normal_range()])
    }
}
...
impl<'a> TryFrom<Cow<'a, str>> for Host<'a> {
    ...
    fn try_from(value: Cow<'a, str>) -> Result<Self, Self::Error> {
        Ok(match value.parse()? {
            HostDetails::Domain(details) => Self::Domain(DomainHost {host: value, details}),
            HostDetails::Ipv4  (details) => Self::Ipv4  (Ipv4Host   {host: value, details}),
            HostDetails::Ipv6  (details) => Self::Ipv6  (Ipv6Host   {host: value, details}),
            HostDetails::Opaque(_) => unreachable!(),
            HostDetails::Empty (_) => Self::Empty (EmptyHost::default()),
        })
    }
}
