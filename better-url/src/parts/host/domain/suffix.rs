...
impl DomainHost<'_> {
    ...
    /// Set, insert, or remove the `index`th suffix segment.
    /// # Errors
    /// See [`Self`]'s documentation.
    /// # Examples
    /// ```
    ...
    /// ```
    pub fn set_suffix_segment<'b, T: TryInto<DomainSegment<'b>>>(&mut self, index: isize, value: Option<T>) -> Result<bool, SetDomainError> where SetDomainError: From<T::Error> {
    ...
    /// Insert new segments starting at the `index`th suffix segment.
    /// # Errors
    /// See [`Self`]'s documentation.
    /// # Examples
    /// ```
    ...
    /// let mut domain = DomainHost::try_from("example.co").unwrap();
    ///
    /// domain.insert_suffix_segments( 1, "uk" ).unwrap    (); assert_eq!(domain, "example.co.uk"      );
    /// domain.insert_suffix_segments( 1, "aa" ).unwrap    (); assert_eq!(domain, "example.co.aa.uk"   );
    /// domain.insert_suffix_segments( 0, "co" ).unwrap    (); assert_eq!(domain, "example.co.aa.co.uk");
    /// domain.insert_suffix_segments(-1, "123").unwrap_err(); assert_eq!(domain, "example.co.aa.co.uk");
    ///
    /// let mut domain = DomainHost::try_from(".").unwrap();
    /// domain.insert_suffix_segments( 1, "123").unwrap_err();
    /// domain.insert_suffix_segments(-1, "123").unwrap_err();
    /// ```
