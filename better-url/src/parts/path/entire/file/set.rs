...
impl FileSegmentedPath<'_> {
    ...
    /// return;
    ///
    /// let mut path = FileSegmentedPath::new("/abc/def/ghi"); path.set_range(0..=0, ["123"              ]).unwrap(); assert_eq!(path, "/123/def/ghi");
    /// let mut path = FileSegmentedPath::new("/abc/def/ghi"); path.set_range(1..=1, ["123"              ]).unwrap(); assert_eq!(path, "/abc/123/ghi");
    /// let mut path = FileSegmentedPath::new("/abc/def/ghi"); path.set_range(2..=2, ["123"              ]).unwrap(); assert_eq!(path, "/abc/def/123");
    ///
    /// let mut path = FileSegmentedPath::new("/abc/def/ghi"); path.set_range(0..=0, ["123", "456", "789"]).unwrap(); assert_eq!(path, "/123/456/789/def/ghi");
    /// let mut path = FileSegmentedPath::new("/abc/def/ghi"); path.set_range(1..=1, ["123", "456", "789"]).unwrap(); assert_eq!(path, "/abc/123/456/789/ghi");
    /// let mut path = FileSegmentedPath::new("/abc/def/ghi"); path.set_range(2..=2, ["123", "456", "789"]).unwrap(); assert_eq!(path, "/abc/def/123/456/789");
    ///
    /// let mut path = FileSegmentedPath::new("/abc/def/ghi"); path.set_range(0..=0, ["."                ]).unwrap(); assert_eq!(path, "/def/ghi" );
    /// let mut path = FileSegmentedPath::new("/abc/def/ghi"); path.set_range(1..=1, ["."                ]).unwrap(); assert_eq!(path, "/abc/ghi" );
    /// let mut path = FileSegmentedPath::new("/abc/def/ghi"); path.set_range(2..=2, ["."                ]).unwrap(); assert_eq!(path, "/abc/def/");
    ///
    /// let mut path = FileSegmentedPath::new("/abc/def/ghi"); path.set_range(0..=0, [".."               ]).unwrap(); assert_eq!(path, "/def/ghi" );
    /// let mut path = FileSegmentedPath::new("/abc/def/ghi"); path.set_range(1..=1, [".."               ]).unwrap(); assert_eq!(path, "/ghi"     );
    /// let mut path = FileSegmentedPath::new("/abc/def/ghi"); path.set_range(2..=2, [".."               ]).unwrap(); assert_eq!(path, "/abc/"    );
    ///
    /// let mut path = FileSegmentedPath::new("/abc/def/ghi"); path.set_range(0..=0, ["..", ".."         ]).unwrap(); assert_eq!(path, "/def/ghi" );
    /// let mut path = FileSegmentedPath::new("/abc/def/ghi"); path.set_range(1..=1, ["..", ".."         ]).unwrap(); assert_eq!(path, "/ghi"     );
    /// let mut path = FileSegmentedPath::new("/abc/def/ghi"); path.set_range(2..=2, ["..", ".."         ]).unwrap(); assert_eq!(path, "/"        );
    ///
    /// let mut path = FileSegmentedPath::new("/abc/def/ghi"); path.set_range(0..=0, ["..", "123", ".."  ]).unwrap(); assert_eq!(path, "/def/ghi" );
    /// let mut path = FileSegmentedPath::new("/abc/def/ghi"); path.set_range(1..=1, ["..", "123", ".."  ]).unwrap(); assert_eq!(path, "/ghi"     );
    /// let mut path = FileSegmentedPath::new("/abc/def/ghi"); path.set_range(2..=2, ["..", "123", ".."  ]).unwrap(); assert_eq!(path, "/abc/"    );
    /// ```
    pub fn set_range<'a, T: Into<FilePathSegment<'a>>, I: IntoIterator<Item = T>, B: RangeBounds<isize>>(&mut self, range: B, iter: I) -> Result<bool, SetPathError> {
