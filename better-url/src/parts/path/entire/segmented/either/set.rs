...
impl SegmentedPath<'_> {
    ...
    /// Either [`SpecialNotFileSegmentedPath::set_segments`], [`FileSegmentedPath::set_segments`], or [`NonSpecialSegmentedPath::set_segments`].
    /// # Errors
    /// If the call to [`SpecialNotFileSegmentedPath::set_segments`] returns an error, that error is returned.
    ///
    /// If the call to [`FileSegmentedPath::set_segments`] returns an error, that error is returned.
    ///
    /// If the call to [`NonSpecialSegmentedPath::set_segments`] returns an error, that error is returned.
    pub fn set_segments<'a, T: Into<SpecialNotFilePathSegment<'a>> + Into<FilePathSegment<'a>> + Into<NonSpecialPathSegment<'a>>, I: IntoIterator<Item = T>, B: RangeBounds<isize>>(&mut self, range: B, iter: I) -> Result<bool, SetPathError> {
        match self {
            Self::SpecialNotFile(x) => x.set_segments(range, iter),
            Self::File          (x) => x.set_segments(range, iter),
            Self::NonSpecial    (x) => x.set_segments(range, iter),
        }
    }
