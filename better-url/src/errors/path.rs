...
/// Returned when failing to parse an empty path.
#[derive(Debug, Error)]
#[error("Failed to parse an invalid empty path.")]
pub struct InvalidEmptyPath;
...
pub enum SetPathError {
    ...
    /** [`InvalidEmptyPath`]. **/ #[error(transparent)] InvalidEmptyPath(#[from] InvalidEmptyPath),
