...
impl BetterDomainHost<'_> {
    ...
    ///
    /// let mut domain = BetterDomainHost::try_from("example.com").unwrap();
    ///
    ...
    pub fn set_suffix(&mut self, value: Option<&str>) -> Result<(), SetDomainError> {
        match value {
            Some(value) => {
                if self.host.len() - self.suffix().len() + value.len() > u32::MAX as usize {
                    Err(TooLong)?;
                }
                ...
                self.host.replace_substr(self.suffix(), value);
            },
            None => match (self.get(..=DomainPart::Middle), self.is_fqdn()) {
                (None   , false) => Err(CantBeEmpty)?,
                (None   , true ) => self.host = ".".into(),
                (Some(x), false) => self.host.retain_substr(x),
                (Some(_), true ) => self.host.retain_range(..self.details.suffix_start()),
            }
        }
    ...
    /// assert_eq!(domain.as_str(), "example.123.uk");
    ///
    /// let mut domain = BetterDomainHost::try_from(".").unwrap();
    ...
    /// use better_url::prelude::*;
    ///
    /// let mut domain = BetterDomainHost::try_from("example.co").unwrap();
    ///
