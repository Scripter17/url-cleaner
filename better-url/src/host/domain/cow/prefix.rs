...
impl BetterDomainHost<'_> {
    ...
    pub fn set_prefix(&mut self, value: Option<&str>) -> Result<(), SetDomainError> {
        match (self.prefix(), value) {
            ...
            (Some(prefix), Some(value)) => {
                if self.host.len() - prefix.len() + value.len() > u32::MAX as usize {
                    Err(TooLong)?
                }
    ...
    pub fn set_prefix_segment(&mut self, index: isize, value: Option<&str>) -> Result<(), SetDomainError> {
        match value {
            Some(value) => match self.try_prefix_segment(index) {
                Ok(segment) => {
                    if self.host.len() - segment.len() + value.len() > u32::MAX as usize {
                        Err(TooLong)?;
                    }
    ...
    /// use better_url::prelude::*;
    ///
    /// let mut domain = BetterDomainHost::try_from("www.example.com").unwrap();
    ///
    ...
    pub fn replace_prefix_segment(&mut self, index: isize, value: Option<&str>) -> Result<(), SetDomainError> {
        ...
        match value {
            Some(value) => {
                if self.host.len() - sl + value.len() > u32::MAX as usize {
                    Err(TooLong)?;
                }
