...
echo ""
cat default-config.json | jq '.docs.flags | to_entries | map(("- `" + .key + "`: " + .value)) | join("\n")' -r
echo ""
...
echo ""
cat default-config.json | jq '.docs.vars | to_entries | map(("- `" + .key + "`: " + .value)) | join("\n")' -r
echo ""
...
echo ""
cat default-config.json | jq '.docs.environment_vars | to_entries | map(("- `" + .key + "`: " + .value)) | join("\n")' -r
echo ""
...
echo ""
cat default-config.json | jq '.docs.sets | to_entries | map(("- `" + .key + "`: " + .value)) | join("\n")' -r
echo ""
...
echo ""
cat default-config.json | jq '.docs.lists | to_entries | map(("- `" + .key + "`: " + .value)) | join("\n")' -r
echo ""
