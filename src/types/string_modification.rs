...
pub enum StringModification {
    ...
    ///     join: "---".to_string()
    /// }.apply(&mut x, &Params::default()).unwrap();
    ...
    ///     split: " ".to_string(),
    ///     n: 1,
    ///     modification: Box::new(StringModification::Set("abc".to_string()))
    /// }.apply(&mut x, &Params::default()).unwrap();
    /// assert_eq!(x, "abc abc");
    /// ```
    /// # Errors
    ...
    ///     split: " ".to_string(),
    ///     ns: vec![1, 2],
    ///     modification: Box::new(StringModification::Set("a b c".to_string()))
    /// }.apply(&mut x, &Params::default()).unwrap();
...
impl StringModification {
    ...
    pub fn apply(&self, to: &mut String, params: &Params) -> Result<(), StringModificationError> {
        ...
        match self {
            ...
            Self::TryElse{r#try, r#else} => r#try.apply(to, params).or_else(|try_error| r#else.apply(to, params).map_err(|else_error| StringModificationError::TryElseError {try_error: Box::new(try_error), else_error: Box::new(else_error)}))?,
            ...
            Self::KeepSegmentRange {split, start, end} => *to=neg_vec_keep(to.split(split), *start, *end).ok_or(StringModificationError::SegmentRangeNotFound)?.join(split),
            ...
            Self::ModifyNthSegment {split, n, modification} => {
                ...
                modification.apply(&mut temp, params)?;
