...
pub enum StringModification {
    ...
    /// # use url_cleaner::types::StringModification;
    /// let mut x = "abcdef".to_string();
    /// assert!(StringModification::ReplaceRange{start: 6, end: 7, replace: "g".to_string()}.apply(&mut x).is_err());
    /// assert_eq!(&x, "abcdef");
    /// assert!(StringModification::ReplaceRange{start: 1, end: 4, replace: "...".to_string()}.apply(&mut x).is_ok());
    /// assert_eq!(&x, "a...ef");
    /// ```
    ReplaceRange{
...
impl StringModification {
    ...
    pub fn apply(&self, to: &mut String) -> Result<(), StringError> {
        match self {
            ...
            Self::ReplaceRange{start, end, replace} => if to.is_char_boundary(*start) && to.is_char_boundary(*end) {to.replace_range(start..end, replace)} else {Err(StringError::InvalidSlice)?}, // Why does `String::try_replace_range` not exist???
