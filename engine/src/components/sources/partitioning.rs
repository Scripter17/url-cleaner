...
pub enum PartitioningSource {
    /// Get from [`Params::Partitionings`]
    /// # Errors
    #[doc = edoc!(geterr(StringSource), getnone(StringSource, PartitioningSourceError))]
    Params(StringSource)
}

/// The enum of errors [`PartitioningSource::get`] can return.
#[derive(Debug, Error)]
...
impl<'de> Visitor<'de> for PartitioningSourceVisitor {
    ...
    fn expecting(&self, formatter: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(formatter, "a string or another variant written normally.")
    }
