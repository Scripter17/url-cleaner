...
pub enum StringSource {
    ...
    /// # Errors
    /// If [`Self::AssertMatches::matcher`] is unsatisfied, returns the error [`StringSourceError::AssertMatchesFailed`].
    AssertMatches {
...
pub enum StringSourceError {
    ...
    /// [`AssertMatchesError`].
    #[error(transparent)]
    AssertMatchesError(#[from] AssertMatchesError),
    /// [`TryElseError`].
    #[error(transparent)]
    ...
    /// [`Box<StringMatcherError>`].
    #[error(transparent)]
    StringMatcherError(#[from] Box<StringMatcherError>),
    /// [`PartitioningSourceError`].
    #[error(transparent)]
    PartitioningSourceError(#[from] Box<PartitioningSourceError>),
    /// [`RegexExpansionError`].
    #[error(transparent)]
    ...
    /// [`HttpRequestSourceError`].
    #[cfg(feature = "http")]
    #[error(transparent)]
    HttpRequestSourceError(#[from] Box<HttpRequestSourceError>),
    /// [`reqwest::Error`].
    #[cfg(feature = "http")]
    #[error(transparent)]
    ...
    /// An arbitrary [`std::error::Error`] for use with [`StringSource::Extern`].
    #[error(transparent)]
...
impl StringSource {
    ...
    fn _get<'j: 't, 't>(&'j self, task_state: &'t TaskState<'j>, args: Option<&'j FunctionArgs>) -> Result<Option<Cow<'t, str>>, StringSourceError> {
        Ok(match self {
            ...
            Self::NoneTo {value, if_none} => match get!(?value) {
                None => get!(?if_none),
                x => x
            },
            Self::NoneToEmpty(value) => match get!(?value) {
                None => Some("".into()),
                x => x
            },
            Self::EmptyTo {value, if_empty} => match get!(?value) {
                Some(x) if x.is_empty() => get!(?if_empty),
                x => x
            },
            Self::EmptyToNone(value) => match get!(?value) {
            ...
            Self::AssertMatches {value, matcher, message} => {
                ...
                match matcher.check(task_state, args, ret.as_deref())? {
                    true  => ret,
                    false => Err(AssertMatchesError(get!(?message).unwrap_or_default().into_owned()))?
                }
            },
            ...
            Self::JobSourceHostPart(part) => task_state.job.context.source_host.as_ref().and_then(|host| part.get(host)).map(Into::into),
