...
impl From<&str> for JsonSource {
    fn from(value: &str) -> Self {
        value.to_string().into()
    }
}

impl From<String> for JsonSource {
...
impl Serialize for JsonSource {
    fn serialize<S: serde::ser::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        <JsonSource>::serialize(self, serializer)
    }
}

#[allow(clippy::missing_errors_doc, reason = "Who cares?")]
impl<'de> Deserialize<'de> for JsonSource {
    fn deserialize<D: serde::de::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        struct V;

        impl<'de> serde::de::Visitor<'de> for V {
            type Value = JsonSource;
...
impl From<Value> for JsonSource {
