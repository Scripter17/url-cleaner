...
impl HttpBodyConfig {
    /// Inserts the specified body into a [`reqwest::RequestBuilder`].
    /// # Errors
    /// See each variant of [`Self`] for when each variant returns an error.
    pub fn apply<'j>(&'j self, request: reqwest::RequestBuilder, task_state: &TaskState<'j>, args: Option<&'j FunctionArgs>) -> Result<reqwest::RequestBuilder, HttpBodyConfigError> {
        Ok(match self {
