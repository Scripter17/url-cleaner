...
/// The states the DFA in [`get_html_attribute`] can be in.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub enum GetHtmlAttributeLastBite {
...
/// The current state of the [`get_html_attribute`] DFA.
#[derive(Debug)]
struct GetHtmlAttributeState<'a> {
    /// The input.
    input: &'a str,
...
/// Shorthand.
type LB = GetHtmlAttributeLastBite;
/// Shorthand.
type EK = GetHtmlAttributeSyntaxErrorKind;
...
///
/// If the input doesn't start with a complete HTML start tag, returns the error [`GetHtmlAttributeError::UnfinishedTag`].
/// # Examples
/// ```
/// use url_cleaner_engine::prelude::*;
///
/// assert_eq!(get_html_attribute("<a href='aaa'>"       , "href").unwrap(), Some(Some("aaa" .to_string())));
/// assert_eq!(get_html_attribute("<a href='a&quot;a'>"  , "href").unwrap(), Some(Some("a\"a".to_string())));
/// assert_eq!(get_html_attribute("<a href=\"aaa\">"     , "href").unwrap(), Some(Some("aaa" .to_string())));
/// assert_eq!(get_html_attribute("<a href=\"a&quot;a\">", "href").unwrap(), Some(Some("a\"a".to_string())));
/// assert_eq!(get_html_attribute("<a href=aaa>"         , "href").unwrap(), Some(Some("aaa" .to_string())));
/// assert_eq!(get_html_attribute("<a href=a&quot;a>"    , "href").unwrap(), Some(Some("a\"a".to_string())));
...
/// assert_eq!(get_html_attribute("<a href=a&quot;a     >", "href").unwrap(), Some(Some("a\"a".to_string())));
///
/// assert_eq!(get_html_attribute("<a href=b href='aaa'        >", "href").unwrap(), Some(Some("aaa" .to_string())));
/// assert_eq!(get_html_attribute("<a href=b href='a&quot;a'   >", "href").unwrap(), Some(Some("a\"a".to_string())));
/// assert_eq!(get_html_attribute("<a href=b href=\"aaa\"      >", "href").unwrap(), Some(Some("aaa" .to_string())));
/// assert_eq!(get_html_attribute("<a href=b href=\"a&quot;a\" >", "href").unwrap(), Some(Some("a\"a".to_string())));
/// assert_eq!(get_html_attribute("<a href=b href=aaa          >", "href").unwrap(), Some(Some("aaa" .to_string())));
/// assert_eq!(get_html_attribute("<a href=b href=a&quot;a     >", "href").unwrap(), Some(Some("a\"a".to_string())));
///
/// assert_eq!(get_html_attribute("<a>", "href").unwrap(), None, "1");
///
/// assert_eq!(get_html_attribute("<a href>"                           , "href").unwrap(), Some(None));
///
/// assert_eq!(get_html_attribute("<a href href=\"1\">"                , "href").unwrap(), Some(Some("1".to_string())));
/// assert_eq!(get_html_attribute("<a href href=\"1\" href>"           , "href").unwrap(), Some(None));
/// assert_eq!(get_html_attribute("<a href href=\"1\" href href=\"2\">", "href").unwrap(), Some(Some("2".to_string())));
///
/// assert_eq!(get_html_attribute("<a href=\"1\" href>"                , "href").unwrap(), Some(None));
/// assert_eq!(get_html_attribute("<a href=\"1\" href href=\"2\">"     , "href").unwrap(), Some(Some("2".to_string())));
/// assert_eq!(get_html_attribute("<a href=\"1\" href href=\"2\" href>", "href").unwrap(), Some(None));
///
/// assert_eq!(get_html_attribute("<a href=\"1\">stuff"              , "href").unwrap(), Some(Some("1".to_string())));
/// assert_eq!(get_html_attribute("<a href=\"1\"><a href=\"2\">"     , "href").unwrap(), Some(Some("1".to_string())));
/// assert_eq!(get_html_attribute("<a href=\"1\">stuff<a href=\"2\">", "href").unwrap(), Some(Some("1".to_string())));
/// assert_eq!(get_html_attribute("<a href=\"1\">href=\"2\""         , "href").unwrap(), Some(Some("1".to_string())));
/// ```
pub fn get_html_attribute<'a>(input: &'a str, name: &'a str) -> Result<Option<Option<String>>, GetHtmlAttributeError> {
    let mut state = GetHtmlAttributeState {
        input,
        name,
        last_bite: LB::Data,
        ret: None,
    ...
    for (i, c) in input.chars().enumerate() {
        if let Err(e) = munch(&mut state, i, c) {
            return Err(GetHtmlAttributeError::Syntax {
                index: i,
        ...
        if matches!(state.last_bite, LB::Done) {
            return Ok(match state.ret {
                Some(Some(value)) => Some(Some(unescape_html(value)?)),
                Some(None) => Some(None),
                None => None
            });
        }
    ...
    Err(GetHtmlAttributeError::UnfinishedTag)
}

/// Advance the state of the [`get_html_attribute`] DFA.
fn munch(state: &mut GetHtmlAttributeState, i: usize, c: char) -> Result<(), GetHtmlAttributeSyntaxErrorKind> {
