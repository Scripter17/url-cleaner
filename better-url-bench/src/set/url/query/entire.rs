...
/// Benchmark setting a URL's query.
///
...
/// Otherwise:
///
/// 2. The second column is the time it takes BetterUrl::set_query to run using the precomputed Query.
///
/// 3. The third column is the time it takes BetterUrl::set_query to run using the raw --value.
///
...
///
/// 1. The time it took to set the query using the precomputed Query.
///
...
///
/// 4. The time it took to set the query using the raw --value.
///
    ...
    pub fn r#do(self) {
        ...
        for (i, line) in std::io::stdin().lock().lines().map(Result::unwrap).enumerate() {
            ...
            match BetterUrl::new(&line) {
                Ok(url) => {
                    let value = MaybeQuery::new(&self.value, url.query_type());
                    ...
                    for mut url in urls {
                        let _ = url.set_query(self.value.as_deref());
                    }
                    ...
                    if servo {
                        match url::Url::parse(&line) {
                            Ok(url) => {
                                ...
                                for mut url in urls {
                                    url.set_query(self.value.as_deref());
                                }
                    ...
                    if ada {
                        match ada_url::Url::parse(&line, None) {
                            Ok(url) => {
                                ...
                                for mut url in urls {
                                    url.set_search(self.value.as_deref());
                                }
