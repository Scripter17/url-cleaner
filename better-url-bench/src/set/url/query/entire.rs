...
/// Benchmark setting an entire query.
///
/// The columns are:
///
/// 1. The line number
///
/// 2. The time it takes to BetterUrl::set_query with a precomputed Query
///
/// 3. The time it takes to BetterUrl::set_query with --value as-is
///
...
/// If --servo
///
/// 6. The time it takes to servo::Url::set_query with --value as-is.
///
...
/// If --ada
///
/// 11. The time it takes to ada::Url::set_search with --value as-is.
///
        ...
        for (i, line) in std::io::stdin().lock().lines().map(Result::unwrap).enumerate() {
            ...
            match BetterUrl::new(&line) {
                Ok(url) => {
                    ...
                    let typed = MaybeQuery::new(&self.value, url.query_type());

                    let timer = std::time::Instant::now();

                    for mut url in urls {
                        let _ = url.set_query(&typed);
                    }
                    ...
                    for mut url in urls {
                        let _ = url.set_query(&self.value);
                    }
